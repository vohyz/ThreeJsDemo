...
import datetime
import json
from flask_socketio import Namespace, emit, SocketIO, disconnect, join_room, leave_room
...
class MyCustomNamespace(Namespace):
    ...
    def on_my_name(self, data):       # 登录成功 
        sid = request.sid
        print('userName:', data)
        user = User(sid, data)
        self.userPool[sid] = user
        self.nameTable[sid] = data
        emit('my_name', 'ok', room=sid)
        # self.pool[self.user_id] = (self.sid, self)

    def on_create_room(self):
        room = request.sid[-3:-1]+self.nameTable[request.sid]
        join_room(room)
        self.roomTable[request.sid] = room
        emit('create_room', room, room=room)

    def on_join_room(self, data):
        if data in self.roomTable.values():
            join_room(data)
            self.roomTable[request.sid] = data
            emit('join_room', self.nameTable[request.sid], room=data)

    
    def on_operate(self, data):
        aim = ''
        for i in self.roomTable:
            if self.roomTable[i] == self.roomTable[request.sid] and i!= request.sid:
                aim = i
        emit('operate', data, room=aim)

    def on_peng(self, data):
        aim = ''
        for i in self.roomTable:
            if self.roomTable[i] == self.roomTable[request.sid] and i!= request.sid:
                aim = i
        emit('peng', data, room=self.roomTable[request.sid])
...
if __name__ == '__main__':
    socketio.run(app,debug=True,host='127.0.0.1',port=9878)